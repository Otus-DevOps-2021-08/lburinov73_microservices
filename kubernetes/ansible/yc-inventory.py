...
def cmd_list(sdk, folder_id):
    ...
    for instance in instances.instances:
        ...
        if instance.status != 2 or str(nat_ipv4) == "":
            continue
        ansible_group = instance.labels.pop("ansible_group", "default")
        groups.setdefault(ansible_group,
                          {}).setdefault("hosts", []).append(instance.name)
        groups.setdefault("all", {}).setdefault("hosts",
                                                []).append(instance.name)
        meta_hostvars[instance.name] = {
            "ansible_host": nat_ipv4,
            **instance.labels
        }
